...
- A self-hosted GitHub Actions runner on the UAT server deploys the new `IMAGE_TAG`
...
3. Install and register the GitHub self-hosted runner on the server
4. Confirm the Cloudflare tunnel token and UAT domain are ready
...
- Check out the repository on the self-hosted runner
- Copy `docker-compose.yml` and `nginx/nginx.conf` into `/opt/stockcutoff-uat`
